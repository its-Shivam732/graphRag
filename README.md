...
├── project/
│   └── plugins.sbt                # SBT plugins (assembly, scalafmt)
│
├── modules/                       # Library modules (no main class)
│   ├── core/                      # Shared data models
│   │   └── src/main/scala/com/graphrag/core/
│   │       ├── models/
│   │       │   ├── Models.scala           # Chunk, Concept, Mentions, Relation
│   │       │   └── GraphModels.scala      # GraphNode, GraphEdge
│   │       └── config/
│   │           └── Config.scala           # Environment configuration
│   │
│   ├── ingestion/                 # Data ingestion
│   │   └── src/main/scala/com/graphrag/ingestion/
│   │       ├── ChunkSource.scala          # JSONL reader (local/S3)
│   │       └── ChunkNormalizer.scala      # Text preprocessing
│   │
│   ├── llm/                       # LLM integration
│   │   └── src/main/scala/com/graphrag/llm/
│   │       ├── OllamaClient.scala         # Async HTTP client for Ollama
│   │       ├── ConceptExtractor.scala     # Stanford NLP extractor
│   │       ├── LLMConceptRefiner.scala    # LLM prompt builder
│   │       ├── AsyncConceptExtraction.scala # Flink async function
│   │       ├── RelationScorer.scala       # LLM relation scoring
│   │       ├── RelationExtractionStage.scala # Flink pipeline stage
│   │       └── CooccurrenceFinder.scala   # Concept pair detection
│   │
│   ├── neo4j/                     # Neo4j integration
│   │   └── src/main/scala/com/graphrag/neo4j/
│   │       ├── Neo4jClient.scala          # Thread-safe Neo4j client
│   │       ├── Neo4jSinks.scala           # Flink sinks (batch writing)
│   │       └── GraphProjectionStage.scala # Flink → Neo4j projection
│   │
│   └── api/                       # REST API module
│       └── src/main/scala/com/graphrag/api/
│           ├── models/
│           │   └── ApiModels.scala        # Request/response models
│           ├── services/
│           │   ├── QueryService.scala     # Natural language queries
│           │   ├── ExploreService.scala   # Graph exploration
│           │   ├── EvidenceService.scala  # Evidence retrieval
│           │   └── ExplainService.scala   # Query tracing
│           ├── controllers/
│           │   ├── QueryController.scala
│           │   ├── ExploreController.scala
│           │   ├── EvidenceController.scala
│           │   ├── ExplainController.scala
│           │   └── JobStatusController.scala
│           └── GraphRAGApiServer.scala    # Main HTTP server
│
├── flink-job/                     # Deployable Flink job
│   ├── src/main/scala/com/graphrag/
│   │   └── GraphRAGJob.scala              # Main Flink pipeline
│   └── target/scala-2.12/
│       └── graphrag-pipeline.jar          # Built JAR (~500MB)
│
├── api-service/                   # Deployable API service
│   └── target/scala-2.12/
│       └── graphrag-api-service.jar       # Built JAR (~45MB)
│
├── deploy/                        # Deployment configurations
│   ├── Dockerfile.flink                   # Flink job Docker image
│   ├── Dockerfile.api                     # API service Docker image
│   └── ollama-daemonset.yaml              # Kubernetes Ollama deployment
│
└── deployments/                   # Kubernetes manifests
    ├── job-graph-rag.yaml                 # Flink job deployment
    ├── configmap.yaml                     # Environment config
    └── secrets.yaml                       # Neo4j credentials (template)
```
